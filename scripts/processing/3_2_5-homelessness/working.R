scrape_chain_data <- function(url, release_year) {
  ...
  if (dir.exists(paste0('data/raw-data/3_2_5-homelessness/', release_year))) {
    ...
    if (user_confirm=='y') {
      write_csv(df, paste0('data/raw-data/3_2_5-homelessness/', release_year,'/CHAIN_rough_sleeping.csv'))
      unlink(temp)
    }
    else {
      stop(
  ...
  #if (!dir.exists(paste0('data/raw-data/3_2_5-homelessness/', release_year))) {
    dir.create(paste0('data/raw-data/3_2_5-homelessness/', release_year))
    write_csv(df, paste0('data/raw-data/3_2_5-homelessness/', release_year,'/CHAIN_rough_sleeping.csv'))
    unlink(temp)
  }
...
scrape_chain_data(url, release_year)
